...
баг с custom dict в CLI
--custom_dict в CLI передаётся как str, а Scanner ждёт список слов
Сейчас в Scanner при передаче строки наподобие "весна,мир,любовь" вы получите set из символов {в,е,с,н,а,…}. Нужно парсить файл/список и передавать List[str]. Плюс логически лучше отделить «словарь для префиксов/соседей» от «целевых слов для поиска».

неверно Аргумент --vicinity задокументирован, но в коде окрестности берутся не так
В _get_vicinity используется n_gram_size вместо self.vicinity_range для среза, из‑за чего ширина окрестностей плавает. Нужно резать ровно по vicinity_range.
